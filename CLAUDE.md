...
on `macos-latest` for `aarch64-apple-darwin` (Apple Silicon only - we don't
ship Intel) via `tauri-apps/tauri-action`, which uploads the `.dmg` and
`.app.tar.gz` artifacts to a GitHub Release named after the tag.

The release notes are generated by the workflow itself (`releaseBody` in
